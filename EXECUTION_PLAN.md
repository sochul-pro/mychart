...
### TASK-024: 상대 수익률 비교 차트 ✅
**상태**: 완료 (2026-02-28, #74)
**설명**: 복수 종목(주식, ETF, 지수)의 수익률을 하나의 차트에 오버레이하여 비교
**범위**:
- 새 페이지: `/compare` (별도 메뉴)
- 복수 종목을 하나의 라인 차트에 오버레이
- 시작 시점 = 0%, 이후 변동률(%)로 정규화
- 기간 선택: 1M, 3M, 6M(기본), 1Y, 2Y
- 최대 10개 종목 비교
- 지수 지원: KOSPI, KOSDAQ, 코스피200, 코스닥150
- 비교 세트 저장/불러오기 (최대 10개, 이름 지정)
- 종목 추가: 검색창에서 직접 검색

**산출물**:
- 타입: `src/types/comparison.ts`
- DB 모델: `ComparisonSet` (prisma/schema.prisma)
- 정규화 유틸: `src/lib/comparison/normalize.ts`
- API: `src/app/api/compare/` (data, sets, sets/[id])
- API: `src/app/api/indices/` (목록, [code]/ohlcv)
- 훅: `src/hooks/useComparison.ts`, `useComparisonSets.ts`
- 컴포넌트: `src/components/compare/` (Chart, Legend, PeriodSelector, SymbolSearch, SetManager)
- 페이지: `src/app/(main)/compare/page.tsx`

**버그 수정**:
- 차트 라인 색상과 범례 색상 불일치 수정 (`symbols.sort()` 원본 배열 변경 문제)
- 저장된 세트 불러올 때 종목명 미표시 수정 (API 데이터에서 종목명 업데이트)
