...
### MAINT-009: 시장 지표 위젯 버그 수정 및 안정성 개선 ✅
**설명**: 대시보드 시장 심리 지표 위젯 버그 수정
**범위**:
- KOSPI/KOSDAQ 숫자 파싱 버그 수정
  - 네이버 API 응답이 쉼표 포함 문자열 ("4,840.74")
  - `parseFloat("4,840.74")` → 4 (쉼표에서 파싱 중단)
  - `.replace(/,/g, '')` 적용하여 정상 파싱
- VIX API 지수 백오프 재시도 로직 추가
  - 429 에러 시 1초 → 2초 → 4초 대기 후 재시도
  - 최대 3회 재시도
  - 네트워크 에러도 동일하게 처리
- VIX 데이터 소스 Yahoo Finance로 원복 (30일 히스토리 복원)

**산출물**:
- `src/app/api/market-sentiment/route.ts` (수정)

**완료**: 2026-01-16 (#70)

---

### MAINT-010: 대시보드 관심종목 UI 개선 ✅
**설명**: 대시보드 관심종목 섹션을 가격/등락율 표시로 개선
**범위**:
- 그룹별 표시 → 종목 리스트 (그룹 없음)
- 종목명 + 현재가 + 등락율 실시간 표시
- useQueries로 최대 5종목 병렬 시세 조회
- 30초마다 시세 자동 갱신
